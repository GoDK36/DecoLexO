...
# 종성 리스트. 00 ~ 27 + 1(1개 없음)
final = [' ', 'ㄱ', 'ㄲ', 'ㄳ', 'ㄴ', 'ㄵ', 'ㄶ', 'ㄷ', 'ㄹ', 'ㄺ', 'ㄻ', 'ㄼ', 'ㄽ', 'ㄾ', 'ㄿ', 'ㅀ', 'ㅁ', 'ㅂ', 'ㅄ', 'ㅅ', 'ㅆ', 'ㅇ',
         'ㅈ', 'ㅊ', 'ㅋ', 'ㅌ', 'ㅍ', 'ㅎ']
...
class Ui_Deco_LexO (object):
    def setupUi(self, Deco_LexO):
        ...
        self.actionMerge_files = QtWidgets.QAction (Deco_LexO)
    ...
    def retranslateUi(self, Deco_LexO):
        ...
        self.Edit_function_tab.setTabText (self.Edit_function_tab.indexOf (self.Add_tab),
                                           _translate ("Deco_LexO", "Add"))
        ...
        self.Edit_function_tab.setTabText (self.Edit_function_tab.indexOf (self.Remove_tab),
                                           _translate ("Deco_LexO", "Remove"))
        ...
        self.Edit_function_tab.setTabText (self.Edit_function_tab.indexOf (self.Replace_tab),
                                           _translate ("Deco_LexO", "Replace"))
        ...
        self.Edit_function_tab.setTabText (self.Edit_function_tab.indexOf (self.Irreg_tab),
                                           _translate ("Deco_LexO", "Irregular"))
    ...
    def openFiles(self):
        ...
        try:
            fname = QtWidgets.QFileDialog.getOpenFileName (None, 'Open CSV file', '',
                                                           "CSV File (*.csv);; DIC File (*.dic);; FLX.DIC File (*flx.dic)")
            ...
            # flx 형태 슬라이싱
            flxfileform = sfileloc.split('.')[0][-3:]
    ...
    def save_as(self):
        ...
        try:
            ...
            sname = QtWidgets.QFileDialog.getSaveFileName (None, 'Save Location', '',
                                                           'CSV File (*.csv);; DIC File (*.dic);; FLX.DIC File (*.flx.dic)')
            ...
            # flx 형태 슬라이싱
            flxfileform = sfileloc.replace('.flx.dic','flx.dic').split('.')[0][-3:]
            ...
            if flxfileform == 'flx':
                col_nme = handle_df.columns.tolist ()
                result_df = result_df[col_nme]
                return (inflec_df2dic (result_df, sfileloc.replace('.flx.dic','flx.dic')), os.startfile (sfileloc.replace('.flx.dic','flx.dic')))
            if sfileform == 'dic':
                col_nme = handle_df.columns.tolist ()
                result_df = result_df[col_nme]
    ...
    def Save_current_table_function(self):
        ...
        try:
            ...
            if flxfileform == 'flx':
                col_nme = handle_df.columns.tolist ()
                current_table = current_table[col_nme]
                return (inflec_df2dic (current_table, sfileloc.replace('.flx.dic','flx.dic')), os.startfile (sfileloc.replace('.flx.dic','flx.dic')))
            if sfileform == 'dic':
                col_nme = handle_df.columns.tolist ()
                current_table = current_table[col_nme]
    ...
    def filter_function(self):
        ...
        try:
            ...
            if self.FInfo_Input.text () != '':
                statelis = [0]
                FComboDict = {'Equals': 1, 'Starts with': 2, 'Ends with': 3, 'Contains': 4}
                ...
                if self.FInfo_Colname.text () != '':
                    ...
                    if statelis[0] == 2:
                        filtered_df = info_starts_with (filtered_df, self.FInfo_Colname.text (),
                                                        self.FInfo_Input.text (),
                                                        original_index)
            ...
            if self.FFirst_1.text () != '' or self.FFirst_2.text () != '' or self.FFirst_3.text () != '':
                filtered_df = First_Syllable (filtered_df, self.FFirst_1.text (), self.FFirst_2.text (),
                                              self.FFirst_3.text (), original_index)
            ...
            if self.FSec_1.text () != '' or self.FSec_2.text () != '' or self.FSec_3.text () != '':
                filtered_df = Second_Syllable (filtered_df, self.FSec_1.text (), self.FSec_2.text (),
                                               self.FSec_3.text (),
                                               original_index)
            ...
            if self.FSecL_1.text () != '' or self.FSecL_2.text () != '' or self.FSecL_3.text () != '':
                filtered_df = Second_to_Last_Syllable (filtered_df, self.FSecL_1.text (), self.FSecL_2.text (),
                                                       self.FSecL_3.text (), original_index)
            ...
            if self.FLast_1.text () != '' or self.FLast_2.text () != '' or self.FLast_3.text () != '':
                filtered_df = Last_Syllable (filtered_df, self.FLast_1.text (), self.FLast_2.text (),
                                             self.FLast_3.text (),
                                             original_index)
    ...
    def Decrypt_func(self):
        ...
        key = hashlib.pbkdf2_hmac (hash_name=self.Dec_Hn.text (), password=self.Dec_Pw.text ().encode ('utf-8'),
                                   salt=self.Dec_salt.text ().encode ('utf-8'), iterations=self.Dec_iter.value ())
    ...
    def Merge_module(self):
        ...
        self.frame.setObjectName ("frame")
        self.label_2 = QtWidgets.QLabel (self.frame)
        self.label_2.setGeometry (QtCore.QRect (10, 0, 351, 31))
        font = QtGui.QFont ()
        font.setFamily ("Arial")
        ...
        self.label_2.setObjectName ("label_2")
        self.MergelistView = QtWidgets.QListView (self.frame)
        self.MergelistView.setGeometry (QtCore.QRect (20, 30, 331, 81))
        self.MergelistView.setObjectName ("listView")
        self.label_3 = QtWidgets.QLabel (self.frame)
        self.label_3.setGeometry (QtCore.QRect (10, 150, 351, 21))
        font = QtGui.QFont ()
        font.setFamily ("Arial")
        font.setPointSize (12)
        font.setBold (False)
        ...
        self.label_3.setObjectName ("label_3")
        self.RuleLineEdit = QtWidgets.QLineEdit (self.frame)
        self.RuleLineEdit.setGeometry (QtCore.QRect (20, 180, 241, 21))
        self.RuleLineEdit.setObjectName ("lineEdit")
        self.MergefilesButton = QtWidgets.QPushButton (self.frame)
        self.MergefilesButton.setGeometry (QtCore.QRect (144, 120, 81, 23))
        self.MergefilesButton.setObjectName ("MergefilesButton")
        self.RuleBrowseButton = QtWidgets.QPushButton (self.frame)
        self.RuleBrowseButton.setGeometry (QtCore.QRect (270, 180, 75, 23))
        self.RuleBrowseButton.setObjectName ("RuleBrowseButton")
        self.MergeStartButton = QtWidgets.QPushButton (self.frame)
        self.MergeStartButton.setGeometry (QtCore.QRect (130, 220, 111, 31))
        font = QtGui.QFont ()
        font.setBold (True)
        font.setWeight (75)
        self.MergeStartButton.setFont (font)
        self.MergeStartButton.setObjectName ("MergeStartButton")
        self.label = QtWidgets.QLabel (self.Merge_window)
        self.label.setGeometry (QtCore.QRect (5, 1, 381, 31))
        font = QtGui.QFont ()
        font.setFamily ("Arial")
        font.setPointSize (12)
        font.setBold (True)
        ...
        self.label.setObjectName ("label")
        QtCore.QMetaObject.connectSlotsByName (self.Merge_window)
        self.Merge_window.setTabOrder (self.MergelistView, self.MergefilesButton)
        self.Merge_window.setTabOrder (self.MergefilesButton, self.RuleLineEdit)
        self.Merge_window.setTabOrder (self.RuleLineEdit, self.RuleBrowseButton)
        self.Merge_window.setTabOrder (self.RuleBrowseButton, self.MergeStartButton)

        _translate = QtCore.QCoreApplication.translate
        self.Merge_window.setWindowTitle (_translate ("Form", "Merge"))
        self.label_2.setText (_translate ("Form", "Files to merge:"))
        self.label_3.setText (_translate ("Form", "Rules (.csv):"))
        self.MergefilesButton.setText (_translate ("Form", "Browse"))
        self.RuleBrowseButton.setText (_translate ("Form", "Browse"))
        self.MergeStartButton.setText (_translate ("Form", "Merge"))
        self.RuleLineEdit.setText (_translate ("Form", "Input Rules File..."))
        self.label.setText (_translate ("Form", "Merge Function"))

        self.MergefilesButton.released.connect (self.open_merge_files)
    ...
    def open_merge_files(self):
        global merge_fname
        try:
            merge_fname = QtWidgets.QFileDialog.getOpenFileNames (None, 'Open .csv files', '', "CSV Files(*.csv)")
            ViewModel = QtGui.QStandardItemModel ()
            for i in merge_fname[0]:
                ViewModel.appendRow (QtGui.QStandardItem (i))
            self.MergelistView.setModel (ViewModel)
        except Exception:
            pass

    def open_rule_file(self):
        try:
            rule_fname = QtWidgets.QFileDialog.getOpenFileName (None, 'Open .csv files', '', "CSV Files(*.csv)")
            self.RuleLineEdit.setText ((str (rule_fname)).split ("', '")[0][2:])
        except Exception:
            pass
                            ...
                            # 길이가 같고 단어까지 같으면 입력 받은 mydata가 i번째에 있는지 아님 j(i+1)번째 있는지 확인한다.
                                ...
                                for col_nms in merge_data.columns.tolist ():
                                    if col_data in col_nms:
                                        ...
                                        if my_data == merge_data.loc[i, col_nms] and del_data == merge_data.loc[
                                            j, col_nms]:
                                        ...
                                        elif my_data == merge_data.loc[j, col_nms] and del_data == merge_data.loc[
                                            i, col_nms]:
                                            # 유지할 데이터를 stem_data, 따라오는(지워져야 할) 데이터를 follow_data
